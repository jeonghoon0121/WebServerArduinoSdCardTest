    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          client.println("<main>");
          client.println("<p><strong>오늘의 로또 번호:</strong></p>");
          client.println("<p id='lotto' style='font-size:24px; color:#0078D7;'>버튼을 눌러 확인하세요</p>");
          client.println("<button onclick='generateLotto()'>로또 번호 생성</button>");
          client.println("<script>");
          client.println("function generateLotto() {");
          client.println("  let numbers = [];");
          client.println("  while (numbers.length < 5) {");
          client.println("    let num = Math.floor(Math.random() * 45) + 1;");
          client.println("    if (!numbers.includes(num)) numbers.push(num);");
          client.println("  }");
          client.println("  document.getElementById('lotto').innerText = numbers.join(' , ');");
          client.println("}");
          client.println("</script>");
          client.println("</main>");
